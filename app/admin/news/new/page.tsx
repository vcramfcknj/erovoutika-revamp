...
import RichTextEditor from '@/app/admin/RichTextEditor' // Adjusted path
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-slate-100">Add New Article</h1>
          <p className="text-xs text-gray-500 dark:text-slate-500 mt-0.5 font-mono">Drafting_New_Entry</p>
        </div>
      </div>
        ...
        <div className="col-span-2 space-y-4">
          ...
          <div className="p-5 rounded border bg-white dark:bg-[#0d1526] border-gray-200 dark:border-white/[0.07]">
            ...
            <textarea
              ...
              disabled={isLoading}
              className="w-full rounded border px-3 py-2 text-sm resize-none bg-white dark:bg-white/[0.03] border-gray-200 dark:border-white/[0.08] text-gray-900 dark:text-slate-200 placeholder:text-gray-400 dark:placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-orange-400/40 focus:border-orange-400"
            />
          </div>

          <div className="p-5 rounded border bg-white dark:bg-[#0d1526] border-gray-200 dark:border-white/[0.07]">
            <Label className="text-sm font-semibold mb-2 flex items-center gap-2 text-gray-700 dark:text-slate-300">
              <FileText className="w-4 h-4" /> Full Article Content
            </Label>
            <RichTextEditor
        ...
        <div className="col-span-1 space-y-4">
          <div className="p-5 rounded border bg-white dark:bg-[#0d1526] border-gray-200 dark:border-white/[0.07]">
            ...
            {imagePreview ? (
              ...
              <label className="flex flex-col items-center justify-center w-full aspect-video border-2 border-dashed rounded cursor-pointer transition-colors border-gray-300 dark:border-white/[0.1] bg-gray-50 dark:bg-white/[0.02] hover:bg-orange-50 dark:hover:bg-orange-500/[0.04] hover:border-orange-300">
          ...
          <div className="p-5 rounded border bg-orange-50 dark:bg-orange-500/[0.05] border-orange-200 dark:border-orange-500/20">
            <Label htmlFor="scheduled_date" className="text-sm font-semibold mb-3 flex items-center gap-2 text-orange-900 dark:text-orange-400">
              <Clock className="w-4 h-4" /> Schedule Publishing
            </Label>
            <Input id="scheduled_date" type="datetime-local" value={scheduledDateTime}
              onChange={(e) => { setScheduledDateTime(e.target.value); setErrorMessage(null) }}
              className="text-sm border-orange-300 dark:border-orange-500/30 bg-white dark:bg-white/[0.03]" />
          </div>
        </div>
      </div>

      {/* Sticky footer */}
      <div className="sticky bottom-0 -mx-6 px-6 py-4 flex justify-between items-center bg-white dark:bg-[#080d18] border-t border-gray-200 dark:border-white/[0.07] shadow-lg">
        ...
        <div className="flex gap-2">
          <Button type="button" onClick={() => handleSubmit(false)}
            ...
            className="text-sm bg-orange-500 hover:bg-orange-600 text-white border-none">
            <Clock className="w-4 h-4 mr-1.5" />
            {uploadingImage ? 'Uploading...' : 'Schedule'}
          </Button>
          <Button type="button" onClick={() => handleSubmit(true)}
            disabled={isLoading || uploadingImage}
            className="text-sm bg-blue-600 hover:bg-blue-700 text-white border-none">
            <Send className="w-4 h-4 mr-1.5" />
            {uploadingImage ? 'Uploading...' : isLoading ? 'Publishing...' : 'Publish_Live'}
          </Button>
        </div>
      </div>
    </div>
