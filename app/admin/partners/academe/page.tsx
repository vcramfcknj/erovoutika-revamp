...
import { Plus, Pencil, Trash2, GraduationCap, Search, Globe, ArrowLeft, Terminal, Filter } from 'lucide-react'
import {
  Table,
  TableBody,
  TableCell,
...
function CardCorners({ color = 'rgba(147,51,234,0.3)' }: { color?: string }) {
...
export default function AcademePartnersPage() {
  ...
  const fetchPartners = async () => {
    try {
      const { data, error } = await supabase
        .from('partners')
        .select('*')
        .eq('type', 'academe') // Filtered for Academe
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64 font-mono text-purple-500 animate-pulse">
        SYNCING_ACADEME_NODES...
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 mb-2">
        <Link href="/admin/partners" className="group flex items-center gap-2 text-[10px] font-mono uppercase tracking-[0.2em] text-gray-400 hover:text-purple-500 transition-colors">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gray-200 dark:border-white/[0.07] pb-6">
        <div>
          <div className="flex items-center gap-2 mb-2 font-mono text-[9px] tracking-[0.3em] uppercase text-purple-500">
            <Terminal className="w-3 h-3" />
            <span>Root://Directory/Academe</span>
          </div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-slate-100 tracking-tight flex items-center gap-3">
            Academe Partners
            <span className="text-[10px] font-mono font-normal px-2 py-0.5 rounded bg-purple-500/10 text-purple-500 border border-purple-500/20 uppercase">
        ...
        <Button 
          ...
          className="bg-purple-600 hover:bg-transparent text-white hover:text-purple-500 border border-purple-600 font-mono text-[10px] uppercase tracking-[0.2em] h-10 px-6 transition-all duration-300"
        >
      ...
      <div className="relative bg-white dark:bg-[#0d1526] border border-gray-200 dark:border-white/[0.07] p-4">
        <CardCorners color="rgba(147,51,234,0.2)" />
        <div className="flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="flex items-center gap-2 shrink-0">
             ...
             <div className="h-10 px-4 flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-gray-400 bg-gray-50 dark:bg-white/[0.02] border border-gray-200 dark:border-white/[0.05]">
                ...
                <span>{filteredPartners.length} Nodes</span>
             </div>
          </div>
        </div>
      </div>
      ...
      <div className="relative bg-white dark:bg-[#0d1526] border border-gray-200 dark:border-white/[0.07] overflow-hidden">
        <CardCorners color="rgba(147,51,234,0.1)" />
        <Table>
          <TableHeader className="bg-gray-50 dark:bg-white/[0.02]">
            <TableRow className="border-b border-gray-200 dark:border-white/[0.07] hover:bg-transparent">
              ...
              <TableHead className="font-mono text-[10px] tracking-widest uppercase py-4">Institution_Name</TableHead>
              <TableHead className="font-mono text-[10px] tracking-widest uppercase py-4">Portal</TableHead>
          ...
          <TableBody>
            {filteredPartners.length === 0 ? (
              ...
              filteredPartners.map((item) => (
                <TableRow key={item.id} className="group border-b border-gray-100 dark:border-white/[0.03] hover:bg-purple-500/[0.02] transition-colors">
                  <TableCell>
                    <div className="relative w-10 h-10 rounded border border-gray-100 dark:border-white/[0.05] overflow-hidden bg-white flex items-center justify-center transition-colors group-hover:border-purple-500/30">
                      {item.image_url ? (
                  ...
                  <TableCell>
                    <div className="flex flex-col">
                      ...
                      <span className="text-[9px] font-mono text-gray-400 uppercase tracking-tighter">{item.category || 'Academic_Node'}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    {item.website_url && (
                      <a href={item.website_url} target="_blank" rel="noopener noreferrer" 
                         className="flex items-center gap-1.5 text-[10px] text-purple-500 font-mono uppercase tracking-tighter hover:underline">
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button 
                        ...
                        className="h-8 w-8 p-0 border-gray-200 dark:border-white/[0.05] hover:border-purple-500/50 hover:text-purple-500 transition-all"
                      >
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent className="dark:bg-[#0d1526] dark:border-white/[0.07] border-red-500/20">
          <AlertDialogHeader>
            <AlertDialogTitle className="font-mono text-sm tracking-widest uppercase text-red-500">Purge Node?</AlertDialogTitle>
            <AlertDialogDescription className="text-xs text-gray-500 dark:text-slate-400 leading-relaxed font-mono">
              The institutional record will be wiped from the academic directory.
            </AlertDialogDescription>
          </AlertDialogHeader>
