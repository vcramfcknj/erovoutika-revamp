...
import { Plus, Pencil, Trash2, GraduationCap, Search, Globe, ArrowLeft } from 'lucide-react'
import {
  Table,
  TableBody,
  TableCell,
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Academe Partners</h2>
              <p className="text-gray-600 dark:text-slate-400 text-sm">Manage educational institutions</p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-4 shadow-sm">
        <div className="flex flex-col md:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <Input
              placeholder="Search academe partners..."
              value={searchQuery}
          ...
          {searchQuery && (
            <Button
              ...
              className="h-9 text-sm hover:bg-purple-50 dark:hover:bg-purple-950/30 hover:text-purple-600 hover:border-purple-300 dark:border-slate-600"
            >
              Clear
            </Button>
          )}
        ...
        <div className="mt-3 text-xs text-gray-600 dark:text-slate-400">
          Showing {filteredPartners.length} of {partners.length} academe partners
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 overflow-hidden shadow-sm">
        {filteredPartners.length === 0 ? (
          <div className="text-center py-12">
            <GraduationCap className="w-12 h-12 text-gray-300 dark:text-slate-600 mx-auto mb-3" />
            <p className="text-gray-500 dark:text-slate-400 text-sm font-medium">
              {searchQuery
                ? 'No academe partners found matching your search'
                : 'No academe partners yet'}
            </p>
            <p className="text-gray-400 dark:text-slate-500 text-xs mt-1">
              {searchQuery
                ? 'Try adjusting your search'
                : 'Add your first academe partner to get started'}
            </p>
          </div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow className="dark:border-slate-700">
                ...
                <TableHead className="text-right text-xs dark:text-slate-400">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredPartners.map((item) => (
                <TableRow key={item.id} className="hover:bg-purple-50 dark:hover:bg-purple-950/20 transition-colors dark:border-slate-700">
                  <TableCell>
                    {item.image_url ? (
                      ...
                      <div className="w-12 h-12 rounded bg-purple-50 dark:bg-purple-950/30 flex items-center justify-center">
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Button
                        ...
                        className="h-8 w-8 p-0 hover:bg-purple-50 dark:hover:bg-purple-950/30 hover:text-purple-600 dark:hover:text-purple-400"
                      >
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent className="dark:bg-slate-800 dark:border-slate-700">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-base dark:text-slate-100">Delete Academe Partner?</AlertDialogTitle>
